...
int Cam::cameraStart() {
    ...
    init_parameters.camera_resolution = RESOLUTION::HD720; // Use HD720 video mode
    init_parameters.camera_fps = 15; // Set fps at 15
    init_parameters.depth_mode = DEPTH_MODE::PERFORMANCE; // Use performance depth mode
    init_parameters.coordinate_units = UNIT::MILLIMETER; // Set units in millimeter
    //??
    init_parameters.coordinate_system =
        COORDINATE_SYSTEM::RIGHT_HANDED_Y_UP; // OpenGL's coordinate system is right_handed
    //    COORDINATE_SYSTEM::IMAGE; // Used in OpenCV,Standard coordinates system in computer vision.
...
cv::Mat Cam::getImage(int key) {
    ...
    if (zed.grab(runtime_parameters) == ERROR_CODE::SUCCESS) {
        // Retrieve left image
        zed.retrieveImage(color, VIEW::LEFT);
